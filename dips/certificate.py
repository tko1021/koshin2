...
def _draw_gentei_table(c, d, x0, x1, y_top):
    ...
    def _read(x):
        # 旧データの「目視外飛行」は「目視内飛行」として扱う
        return "目視内飛行" if x == "目視外飛行" else x
    ...
    for i, air in enumerate(GENTEI_ROWS):
        ...
        if air != MULTI:
            continue
        raw = [_read(x) for x in (gmap.get(air) or [])]
        # 先頭は常に「基本」。基本以外は「○○（限定解除）」表記。
        disp = ["基本"] + [f"{x}（限定解除）" for x in raw if x != "基本"]
